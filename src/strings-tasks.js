...
function countVowels(str) {
  ...
  const res = str.match(/[aeiouyAEIOUY]/g);
...
function isPalindrome(str) {
  const ReStr = str.toLowerCase().replace(/[^A-Za-z0-9]/g, '');
  const StrL = ReStr.length;
  for (let i = 0; i < StrL / 2; i += 1) {
    if (ReStr[i] !== ReStr[StrL - 1 - i]) {
      return false;
    }
  }
  return true;
...
function reverseWords(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    ...
    for (let j = 0; j < CurWord.length; j += 1) {
      arr[i] = CurWord.split('').reverse().join('');
    }
  }
  return arr.join(' ');
}
...
function encodeToRot13(str) {
  const RightOrder = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz?!.';
  const BackOrder = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm?!.';
  const arr = str.split(' ');

  for (let i = 0; i < arr.length; i += 1) {
    const CurWorld = arr[i];

    let BackWorld = '';
    for (let j = 0; j < CurWorld.length; j += 1) {
      BackWorld += BackOrder[RightOrder.indexOf(CurWorld[j])];
    }
    arr[i] = BackWorld;
  }
